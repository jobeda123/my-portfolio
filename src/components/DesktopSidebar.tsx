...
export default function DesktopSidebar({ sections }: SidebarProps) {
  ...
  return (
    <aside className="hidden lg:block fixed right-6 top-1/2 transform -translate-y-1/2 z-50" role="navigation" aria-label="Desktop navigation sidebar">
          ...
          {sections.map((section) => {
            ...
            return (
              <li key={section} role="none">
                ...
                >
                  <div className={`transition-all duration-200 ${isActive ? 'scale-110 drop-shadow-sm text-white' : 'group-hover:scale-105 group-hover:text-white text-gray-400'}`}>
                    <Icon 
                      type={iconType} 
